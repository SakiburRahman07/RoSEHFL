...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--strategies",
        nargs="+",
        default=["rose", "shapefl", "fedavg", "fedprox", "gtg_shapley", "q_fedavg"],
        choices=[
            "rose",
            "roseplusplus",
            "rose_median",
...
def accuracy_at_budget(metrics: dict, budget_gb: float) -> float | None:
    for cost, accuracy in zip(metrics.get("cumulative_cost_gb", []), metrics.get("accuracy", [])):
        if cost >= budget_gb:
            return float(accuracy)
    return None


def cost_to_target(metrics: dict, target_accuracy: float) -> float | None:
    for cost, accuracy in zip(metrics.get("cumulative_cost_gb", []), metrics.get("accuracy", [])):
...
def main() -> None:
    ...
    common_budget = args.budget_gb
    if common_budget is None:
        final_costs = [
            result["metrics"]["cumulative_cost_gb"][-1]
            for result in all_results.values()
            if result["metrics"]["cumulative_cost_gb"]
        ]
    ...
    for strategy_name, result in all_results.items():
        ...
        summary[strategy_name] = {
            ...
            "cost_to_target_gb": cost_to_target(metrics, args.target_accuracy),
            "accuracy_at_common_budget": accuracy_at_budget(metrics, common_budget),
            "elapsed_seconds": result["elapsed_seconds"],
        }

    payload = {
        "config": vars(args),
        "common_budget_gb": common_budget,
        "summary": summary,
