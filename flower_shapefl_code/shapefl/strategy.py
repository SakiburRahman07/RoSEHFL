...
from .utils.model_state import batch_norm_state_keys, state_key_indices
...
class RoSEHFLStrategy(ShapeFlStrategy):
    ...
    def _collect_client_state(
    ...
    ) -> Tuple[Dict[int, List[np.ndarray]], Dict[int, int], Dict[int, np.ndarray], int]:
        client_weights: Dict[int, List[np.ndarray]] = {}
        client_sizes: Dict[int, int] = {}
        ...
        for client_proxy, fit_res in results:
            ...
            round_probe_bytes += probe_payload_num_bytes(fit_res.metrics)
        ...
        return client_weights, client_sizes, probe_logits, round_probe_bytes
        ...
        if self.phase == "warmup":
            ...
            for client in clients:
                ...
                if class_prior is not None:
                    node_config["class_prior"] = class_prior
    ...
    def _aggregate_warmup(
        ...
        client_weights, client_sizes, probe_logits, _ = self._collect_client_state(results)
    ...
    def _aggregate_train_rose(
        ...
        for node_id, weights in client_weights.items():
            ...
            if edge_id is None:
                continue
            edge_groups[edge_id]["weights"].append(weights)
        ...
        for edge_id in sorted(self.selected_edges):
            ...
            edge_weights.append(edge_weights_current)
