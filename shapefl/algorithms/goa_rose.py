...
class GreedyNodeAssociationRoSE:
    ...
    def _objective(self, state: Dict[str, object]) -> float:
        ...
        fill_penalty = self._fill_penalty_from_counts(state["edge_member_counts"])
        return communication_total - self.gamma * diversity_total + fill_penalty

    def _effective_min_members(self) -> int:
        if self.edge_min_members <= 0:
            return 0
        required = len(self.edge_aggregators) * self.edge_min_members
        if len(self.all_nodes) < required:
            return 0
        return self.edge_min_members

    def _fill_penalty_from_counts(
        self,
        edge_member_counts: Dict[int, int],
    ) -> float:
        min_members = self._effective_min_members()
        if min_members <= 0 or self.edge_underfill_penalty <= 0.0:
            return 0.0
        inverse_num_edges = 1.0 / max(len(self.edge_aggregators), 1)
        deficit_sum = 0.0
        for edge_id in self.edge_aggregators:
            deficit = max(0, min_members - int(edge_member_counts[edge_id]))
            deficit_sum += float(deficit * deficit)
        return self.edge_underfill_penalty * inverse_num_edges * deficit_sum
        ...
        while remaining:
            ...
            for node_id in remaining:
                ...
                for edge_id in self.edge_aggregators:
                    ...
                    old_fill = self._fill_penalty_from_counts(state["edge_member_counts"])
                    candidate_counts = dict(state["edge_member_counts"])
                    candidate_counts[edge_id] = new_member_count
                    new_fill = self._fill_penalty_from_counts(candidate_counts)
                    delta_fill = new_fill - old_fill
                    delta_objective = (
                        communication_cost
                        - self.gamma * inverse_num_edges * delta_term
                        + delta_fill
                    )
