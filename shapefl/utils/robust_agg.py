...
def trust_edge_aggregate(
    ...
    When ``use_shrinkage`` is True (C4: Bayesian-shrinkage trust), deviations
    are clipped at the ``dev_clip_q`` quantile and a Gamma(``prior_a``,
    ``prior_b``) prior is used to stabilise per-client variance estimates on
    small edges:

        sigma2_i = ((n-1) * dev_i^2 + 2 * b0) / (n + 2 * prior_a - 2)
        alpha_i ∝ (sigma2_i + eps)^{-nu} * size_i^eta * phi_i^xi

    When ``prior_b`` is None it is auto-fit as ``median(dev_clipped^2) *
    ...
    if use_shrinkage:
        ...
        if q < 1.0 and num_clients >= 2:
            ...
            dev_clipped = np.minimum(deviations, dev_cap)
        else:
            dev_clipped = deviations.copy()

        a0 = float(max(prior_a, 1.0 + 1e-3))
        if prior_b is None:
            b0 = float(max(np.median(dev_clipped ** 2), 1e-12)) * (a0 - 1.0)
        else:
            b0 = float(max(prior_b, 0.0))
        denom = float(num_clients + 2.0 * a0 - 2.0)
        if denom <= 0.0:
            denom = 1.0
        sigma2 = ((num_clients - 1) * dev_clipped ** 2 + 2.0 * b0) / denom
