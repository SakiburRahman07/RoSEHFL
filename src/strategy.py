"""
RoSEHFL strategy module for Flower
==================================
This module contains the strategy layer used by the RoSEHFL research codebase.

It provides four strategy families:
...
Round semantics:
    - ShapeFlStrategy: round 1 pretraining + planning, followed by
      hierarchical training rounds.
    - RoSEHFLStrategy: warmup planning followed by adaptive training with
...
def generate_communication_costs(
    ...
    # The topology helper returns costs keyed by (client, edge_idx).
    # In this project, every node can be used as a candidate edge, so
    # edge_idx and node_idx share the same index space.
...
class ShapeFlStrategy(_FlowerStrategyBase):
    ...
    def get_checkpoint_state(self) -> Dict[str, object]:
        state = {
            ...
            "communication_cumulative_cost_gb": float(self.communication_cumulative_cost_gb),
            "metrics_history": self.metrics_history,
            "cid_to_partition": self._cid_mapper.to_checkpoint(),
            "model_size_bytes": int(self.model_size_bytes),
            "reported_baseline_per_round_cost_gb": float(self._reported_baseline_per_round_cost_gb),
            "reported_communication_per_round_cost_gb": float(self._reported_communication_per_round_cost_gb),
    ...
    def load_checkpoint_state(self, state: Dict[str, object]) -> None:
        ...
        self._reported_baseline_per_round_cost_gb = float(state.get("reported_baseline_per_round_cost_gb", self.per_round_cost_gb))
        self._reported_communication_per_round_cost_gb = float(state.get("reported_communication_per_round_cost_gb", self.per_round_cost_gb))
    ...
    def _record_model_transfer_cost(
        ...
        *,
        ...
        self.current_cycle_communication_cost_gb += scaled_cost_from_payload(
            float(base_cost_gb),
            payload,
            self.model_size_bytes,
        )

    def _communication_probe_payload_cost(
        self,
        *,
        probe_payload_bytes: Dict[int, int],
        node_edge_map: Dict[int, int],
    ) -> float:
        """Charge probe-logit traffic across the same hierarchy as model traffic."""
        ...
        for node_id, payload_bytes in probe_payload_bytes.items():
            ...
            if payload <= 0:
                continue
            edge_id = node_edge_map.get(int(node_id))
            if edge_id is None:
                continue
    ...
    def _record_probe_payload_cost(
        ...
        *,
        ...
        total_payload_bytes = sum(
            int(payload_bytes)
            for payload_bytes in probe_payload_bytes.values()
            if int(payload_bytes) > 0
        )
        if total_payload_bytes <= 0:
            return
        self.current_cycle_probe_payload_bytes += int(total_payload_bytes)
        self.current_cycle_communication_cost_gb += self._communication_probe_payload_cost(
            probe_payload_bytes=probe_payload_bytes,
            node_edge_map=node_edge_map,
        )

    def _baseline_cost_for_completed_cycle(self, edge_epochs: int) -> float:
        completed_edge_epochs = int(max(edge_epochs, 0))
        if completed_edge_epochs <= 0:
            return 0.0
        node_edge_cost = sum(
            float(self.c_ne.get((node_id, edge_id), 0.0))
            for edge_id in self.selected_edges
            for node_id in self.edge_nodes[edge_id]
        )
        edge_cloud_cost = sum(float(self.c_ec.get(edge_id, 0.0)) for edge_id in self.selected_edges)
    ...
    def _record_completed_cloud_metrics(
        ...
        *,
        ...
        self.metrics_history["per_round_cost_gb"].append(float(self._reported_baseline_per_round_cost_gb))
        self.metrics_history["cumulative_cost_gb"].append(float(self.cumulative_cost_gb))
        self.metrics_history["baseline_per_round_cost_gb"].append(float(self._reported_baseline_per_round_cost_gb))
        self.metrics_history["baseline_cumulative_cost_gb"].append(float(self.cumulative_cost_gb))
        self.metrics_history["communication_per_round_cost_gb"].append(
            float(self._reported_communication_per_round_cost_gb)
        )
        self.metrics_history["communication_cumulative_cost_gb"].append(
            float(self.communication_cumulative_cost_gb)
        )
    ...
    def _compute_per_round_cost(self):
        ...
        ne = sum(
            self.c_ne.get((n, e), 0.0)
            for e in self.selected_edges
            for n in self.edge_nodes[e]
        )
        ec = sum(self.c_ec.get(e, 0.0) for e in self.selected_edges)
...
class FedAvgFlatStrategy(_FlowerStrategyBase):
    ...
    def _record_metrics(self, server_round: int, accuracy: float, loss: float) -> None:
        ...
        self.metrics_history["cumulative_cost_gb"].append(float(self.cumulative_cost_gb))
        self.metrics_history["baseline_per_round_cost_gb"].append(float(self.per_round_cost_gb))
        self.metrics_history["baseline_cumulative_cost_gb"].append(float(self.cumulative_cost_gb))
        self.metrics_history["communication_per_round_cost_gb"].append(float(self.per_round_cost_gb))
        self.metrics_history["communication_cumulative_cost_gb"].append(float(self.cumulative_cost_gb))
...
class RoSEHFLStrategy(ShapeFlStrategy):
    ...
    def _estimate_plan_cost_gb(self, edge_nodes: Dict[int, List[int]]) -> float:
        ne = sum(
            self.c_ne.get((node_id, edge_id), 0.0)
            for edge_id, nodes in edge_nodes.items()
            for node_id in nodes
        )
        ec = sum(self.c_ec.get(edge_id, 0.0) for edge_id in edge_nodes)
    ...
    def _serialise_metrics(self) -> Dict[str, object]:
        return {
            ...
            "baseline_per_round_cost_gb": [
                float(value) for value in self.metrics_history.get("baseline_per_round_cost_gb", [])
            ],
            "baseline_cumulative_cost_gb": [
                float(value) for value in self.metrics_history.get("baseline_cumulative_cost_gb", [])
            ],
            "communication_per_round_cost_gb": [
                float(value) for value in self.metrics_history.get("communication_per_round_cost_gb", [])
            ],
            "communication_cumulative_cost_gb": [
                float(value) for value in self.metrics_history.get("communication_cumulative_cost_gb", [])
            ],
            "model_payload_bytes": [
    ...
    def load_checkpoint_state(self, state: Dict[str, object]) -> None:
        ...
        self.model_size_bytes = int(state.get("model_size_bytes", self.model_size_bytes))
        self.communication_cumulative_cost_gb = float(
            state.get("communication_cumulative_cost_gb", self.cumulative_cost_gb)
        )
        self._reported_baseline_per_round_cost_gb = float(
            state.get("reported_baseline_per_round_cost_gb", self.per_round_cost_gb)
        )
        self._reported_communication_per_round_cost_gb = float(
            state.get("reported_communication_per_round_cost_gb", self.per_round_cost_gb)
        )
        self._reported_model_payload_bytes = int(state.get("reported_model_payload_bytes", 0))
        self._reported_probe_payload_bytes = int(state.get("reported_probe_payload_bytes", 0))
        self.current_cycle_model_payload_bytes = int(state.get("current_cycle_model_payload_bytes", 0))
    ...
    def _simulate_candidate_cycle(
        ...
        *,
        ...
        for payload_bytes in (probe_payload_bytes or {}).values():
            payload = int(payload_bytes)
            if payload > 0:
        ...
        if include_probe_payload:
            effective_cost_gb += self._communication_probe_payload_cost(
                ...
                node_edge_map=candidate_node_edge,
            )
        ...
        *,
        ...
        if self.planning_objective == "effective":
            evaluated["planning_score_gb"] = float(
                ...
                + evaluated["projected_remaining_rounds"] * evaluated["communication_per_round_cost_gb"]
            )
        else:
            evaluated["planning_score_gb"] = float(evaluated["baseline_per_round_cost_gb"])
        return evaluated
    ...
    def _serialise_candidate_evaluation(candidate: Dict[str, object]) -> Dict[str, object]:
        return {
            ...
            "baseline_per_round_cost_gb": float(candidate.get("baseline_per_round_cost_gb", 0.0)),
            "communication_per_round_cost_gb": float(candidate.get("communication_per_round_cost_gb", 0.0)),
    ...
    def _plan_with_signal(
        ...
        if planning_objective_active == "effective":
            ...
            event["candidate_per_round_cost_gb"] = float(chosen_candidate["baseline_per_round_cost_gb"])
            event["candidate_communication_per_round_cost_gb"] = float(
                chosen_candidate["communication_per_round_cost_gb"]
            )
    ...
    def _aggregate_train_rose(
        ...
        self._record_probe_payload_cost(
            probe_payload_bytes=probe_payload_bytes,
            node_edge_map=self.node_edge,
        )
